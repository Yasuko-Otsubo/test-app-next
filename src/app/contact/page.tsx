...
const Contact: React.FC = () => {
  ...
  type Error = Record<keyof Contact, string>;
  ...
  const show = async () => {
    ...
    if (isValidNameLength) {
      setError((prev) => ({
        ...prev,
        name: "お名前は30字以内で入力してください",
      }));
      isValid = false;
    }
    if (form.name.trim() && isValidNameLength) {
      // 名前が正しい場合、エラーメッセージをクリア
      ...
      <div className={styles.form_page}>
        ...
        <form className={styles.form_area}>
          ...
          <div className={styles.f_input}>
            ...
            {error.email && (
              <p className={styles.f_note_error}>{error.email}</p>
            )}
          </div>
          ...
          <div className={styles.f_btn}>
            <button type="button" className={styles.f_send} onClick={show}>
              送信
            </button>
            <button
              type="button"
              className={styles.f_clear}
              onClick={clearForm}
            >
              クリア
            </button>
